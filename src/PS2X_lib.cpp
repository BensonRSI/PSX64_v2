...
#if CTRL_CLK_LOW
      delayMicroseconds(CTRL_CLK_LOW);
#endif
...
boolean PS2X::read_gamepad(boolean motor1, byte motor2) {
   ...
   for (byte RetryCnt = 0; RetryCnt < 15; RetryCnt++) {
      CMD_SET();
      CLK_SET();
  ...
  /*if (PS2data[1] != 0x73)*/{
      ...
      if ((PS2data[1] & 0xf0) == 0x70){ 
       // Serial.println("got correct answer");
        break;
      }
...
byte PS2X::config_gamepad(uint8_t clk, uint8_t cmd, uint8_t att, uint8_t dat, bool pressures, bool rumble) {
  ...
  boolean ret =read_gamepad(false,0);
   ret |= read_gamepad(false,0);
    
  Serial.print ("read_gamepad returned :");
  Serial.println (ret);
  //see if it talked - see if mode came back. 
  //If still anything but 41, 73 or 79, then it's not talking (GuitarHero clone sends 63)
  if(PS2data[1] != 0x41 && PS2data[1] != 0x42 && PS2data[1] != 0x73 && PS2data[1] != 0x79 && PS2data[1] != 0x63){ 
#ifdef PS2X_DEBUG
    Serial.println("Controller mode not matched or no controller found");
    Serial.print("Expected 0x41, 0x42, 0x63, 0x73 or 0x79, but got ");
...
#ifdef PS2X_DEBUG
    Serial.println("Controller response on type request: ");
    for (int i = 0; i<9; i++) {
